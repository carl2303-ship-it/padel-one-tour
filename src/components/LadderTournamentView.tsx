  ...
  const load = useCallback(async () => {
    ...
    const [ladderRes, teamsRes] = await Promise.all([
      ...
      supabase
        .from('teams')
        .select(
          'id, name, player1_id, player2_id, category_id, registration_source, organizer_review_status, player1:players!teams_player1_id_fkey(id, name, user_id, player_account_id), player2:players!teams_player2_id_fkey(id, name, user_id, player_account_id)'
        )
        .eq('tournament_id', tournament.id)
        .eq('category_id', resolved)
