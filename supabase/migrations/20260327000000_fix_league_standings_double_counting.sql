-- Fix double-counting in league standings:
-- 1. Use DISTINCT ON to prevent processing the same tournament twice
-- 2. Exclude individual players whose name matches a team player in the same tournament
--    (handles duplicate player records with different IDs but same name)
...
BEGIN
  ...
  -- DISTINCT ON prevents processing same tournament twice if duplicate tournament_leagues entries exist
  FOR tournament_record IN
    SELECT DISTINCT ON (tl.tournament_id)
      tl.tournament_id,
      tl.league_category,
      tl.group_filter,
      t.status
  ...
  LOOP
    ...
    -- Also exclude players whose name matches any team player (prevents double-counting
    -- when duplicate player records exist with different IDs but same name)
    FOR player_record IN
      ...
      AND NOT EXISTS (
        ...
        LEFT JOIN players tp1 ON tp1.id = t3.player1_id
        LEFT JOIN players tp2 ON tp2.id = t3.player2_id
        WHERE t3.tournament_id = tournament_record.tournament_id
        AND t3.final_position IS NOT NULL
        AND (LOWER(tp1.name) = LOWER(p.name) OR LOWER(tp2.name) = LOWER(p.name))
      )
    LOOP
