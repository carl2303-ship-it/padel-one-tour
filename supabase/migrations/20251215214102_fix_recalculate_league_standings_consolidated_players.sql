/*
  # Fix League Standings Recalculation - Use Consolidated Players Table

  1. Changes
    - Update recalculate_league_standings function to use only `players` table
    - Remove references to non-existent `individual_players` table
    - Handle both team tournaments and individual tournaments using unified `players` table
    
  2. Key Updates
    - For team tournaments: Get players from teams via player1_id and player2_id
    - For individual tournaments: Get players directly from players table where tournament_id matches
*/

-- Drop existing function
DROP FUNCTION IF EXISTS recalculate_league_standings(uuid);

-- Recreate function using only the consolidated players table
...
BEGIN
  ...
  -- Recalculate from individual players in completed tournaments (using consolidated players table)
  FOR player_record IN
    ...
    WHERE t.league_id = league_uuid
      ...
      AND NOT EXISTS (
        -- Exclude players who are part of teams (to avoid double counting)
