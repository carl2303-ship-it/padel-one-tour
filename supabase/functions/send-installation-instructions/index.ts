...
interface InstallationEmailRequest {
  email: string;
  phoneNumber: string;
  playerName?: string;
}
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const body: InstallationEmailRequest = await req.json();
    const { email, phoneNumber, playerName } = body;

    if (!email || !phoneNumber) {
      return new Response(
        JSON.stringify({ error: 'email and phoneNumber are required' }),
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #f5f5f5;">
  <table role="presentation" style="width: 100%; border-collapse: collapse;">
    ...
    return new Response(
      JSON.stringify({ 
        success: true, 
        message: 'Installation instructions email sent successfully',
    ...
    console.error('Error in send-installation-instructions:', error);
    return new Response(
      JSON.stringify({ 
